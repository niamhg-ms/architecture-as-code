...
const adrId = '1';
const revision = '1';
const expectedAdr = JSON.parse(fs.readFileSync('src/tests/fixtures/adr.json', 'utf8'));
...
describe('Adr Service ', () => {
    ...
    it.each(errorStatusCodes)('should throw an error when backend service returns an unsuccessful status code while retrieving all the ADR IDs', async (errorStatusCode) => {
        const errorMessage = `
        {
    ...
    it.each(errorStatusCodes)('should throw an error when backend service returns an unsuccessful status code while retrieving all the revisions for an ADR', async (errorStatusCode) => {
        const errorMessage = `
        {
            "message": "An Error Occurred",
        }`;
        mock.onGet(`/calm/namespaces/${namespace}/adrs`).reply(errorStatusCode, errorMessage);
        expect(async () => await adrService.fetchAdrRevisions(namespace, adrId)).rejects.toThrowError();
    });

    it('should retrieve an adr revision', async () => {
        mock.onGet(`/calm/namespaces/${namespace}/adrs/${adrId}/revisions/${revision}`).reply(200, expectedAdr);
        const actualAdr  = await adrService.fetchAdr(namespace, adrId, revision);
        expect(actualAdr).toEqual(expectedAdr);
    });

    it.each(errorStatusCodes)('should throw an error when backend service returns an unsuccessful status code while retrieving a specific revision of an ADR', async (errorStatusCode) => {
        const errorMessage = `
        {
            "message": "An Error Occurred",
        }`;
        mock.onGet(`/calm/namespaces/${namespace}/adrs/${adrId}/revisions/${revision}`).reply(errorStatusCode, errorMessage);
        expect(async () => await adrService.fetchAdr(namespace, adrId, revision)).rejects.toThrowError();
    });
